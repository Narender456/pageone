...
export function DrugsDetailsDialog({ open, onOpenChange, drugs, studies = [] }) {
...
const getSelectedStudies = () => {
  if (!drugs?.studies || !Array.isArray(drugs.studies)) {
    return []
  }

  return drugs.studies.map(study => ({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          ...
          <DialogDescription>{drugs ? `Details for "${drugs.drug_name}"` : "Loading..."}</DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm">{drugs.drug_name}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Status</label>
                      <div className="mt-1">
                        <Badge variant="secondary" className={getStatusColor(drugs.isActive)}>
                          {drugs.isActive ? "Active" : "Inactive"}
                        </Badge>
                      </div>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Unique ID</label>
                      <p className="text-sm font-mono">{drugs.uniqueId}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Slug</label>
                      <p className="text-sm font-mono">{drugs.slug}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Created</label>
                      <p className="text-sm">{formatDate(drugs.date_created)}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Last Updated</label>
                      <p className="text-sm">{formatDate(drugs.last_updated)}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{selectedStudies.length} studies assigned to this drug</CardDescription>
                </CardHeader>
                <CardContent>
                  {selectedStudies.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      <p className="text-muted-foreground">No studies assigned to this drug</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div>
                      ...
                      <p className="text-sm font-mono bg-muted p-2 rounded mt-1">{drugs.absoluteUrl}</p>
                    </div>
                  </CardContent>
                </Card>
              )}
