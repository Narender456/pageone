...
export function SponsorDetailsDialog({ open, onOpenChange, sponsor, studies = [] }) {
...
const getSelectedStudies = () => {
  ...
  return sponsor.studies.map(study => ({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          ...
          <DialogDescription>{sponsor ? `Details for "${sponsor.sponsor_name}"` : "Loading..."}</DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm">{sponsor.sponsor_name}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Status</label>
                      <div className="mt-1">
                        <Badge variant="secondary" className={getStatusColor(sponsor.isActive)}>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-mono">{sponsor.uniqueId}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Slug</label>
                      <p className="text-sm font-mono">{sponsor.slug}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Created</label>
                      <p className="text-sm">{formatDate(sponsor.date_created)}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Last Updated</label>
                      <p className="text-sm">{formatDate(sponsor.last_updated)}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{selectedStudies.length} studies assigned to this sponsor</CardDescription>
                </CardHeader>
                <CardContent>
                  {selectedStudies.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      <p className="text-muted-foreground">No studies assigned to this sponsor</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div>
                      ...
                      <p className="text-sm font-mono bg-muted p-2 rounded mt-1">{sponsor.absoluteUrl}</p>
                    </div>
                  </CardContent>
                </Card>
              )}
...
SponsorDetailsDialog.propTypes = {
