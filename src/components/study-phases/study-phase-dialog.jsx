...
export function StudyPhaseDialog({ open, onOpenChange, onSave, studyPhase, title, loading, studies = [] }) {
  ...
  useEffect(() => {
    if (studyPhase) {
      setFormData({
        ...
        selectedStudies: studyPhase.studies?.map((s) => s._id || s.id) || [],
      })
    } else {
      setFormData({
        study_phase: "",
...
useEffect(() => {
  ...
  const validateForm = () => {
    const newErrors = {}
    if (!formData.study_phase.trim()) {
      newErrors.study_phase = "Study phase name is required"
    } else if (formData.study_phase.length > 255) {
      newErrors.study_phase = "Study phase name cannot exceed 255 characters"
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {studyPhase ? "Update the study phase information." : "Add a new study phase to organize your studies."}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="study_phase">Study Phase Name *</Label>
              <Input
                id="study_phase"
                value={formData.study_phase}
                onChange={(e) => setFormData({ ...formData, study_phase: e.target.value })}
                placeholder="Enter study phase name"
                className={errors.study_phase ? "border-destructive" : ""}
              />
              {errors.study_phase && <p className="text-sm text-destructive">{errors.study_phase}</p>}
            </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading}>
              {loading ? "Saving..." : studyPhase ? "Update Study Phase" : "Add Study Phase"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
